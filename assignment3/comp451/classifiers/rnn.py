...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        out, e_cache = word_embedding_forward(captions_in, W_embed)
        h, f_cache = rnn_forward(out, np.dot(features,W_proj) + b_proj, Wx, Wh, b)
        vout, v_cache = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dx = temporal_softmax_loss(vout, captions_out , mask)
        dout, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dx, v_cache)
        dout, dh0, grads["Wx"], grads["Wh"], grads["b"] = rnn_backward(dout,f_cache)
        grads.update({"W_embed": word_embedding_backward(dout,e_cache),"W_proj": np.dot(features.T,dh0),"b_proj":np.sum(dh0,axis=0)})
        if self.gclip > 0:
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        clipped_grads = {}
        for key in grads:
            temp = grads[key] * gclip / np.linalg.norm(grads[key])
            clipped_grads[key] = temp
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        pre_w = np.array([self._start] * N)
        prev_h= np.dot(features,W_proj) + b_proj

        for w in range(max_length):
            out,_ = word_embedding_forward(pre_w, W_embed)
            next_h,_ = rnn_step_forward(out, prev_h, Wx, Wh, b)
            prev_h = next_h
            vout,_ = temporal_affine_forward(np.transpose(np.reshape(next_h,(next_h.shape[0],next_h.shape[1],1)),(0,2,1)),W_vocab, b_vocab)
            captions[:,w]= np.argmax(vout,axis=2)
            pre_w= np.argmax(vout,axis=2).reshape(N)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        for w in range(max_length):
            ...
            prev_h = next_h
            vout,_ = temporal_affine_forward(np.transpose(np.reshape(next_h,(next_h.shape[0],next_h.shape[1],1)),(0,2,1)),W_vocab, b_vocab)
            probs = np.exp(vout[0][0]- np.max(vout[0][0])) / np.exp(vout[0][0]- np.max(vout[0][0])).sum()
            captions[:,w]= np.random.choice(np.arange(len(vout[0][0])), p=probs)
            pre_w= captions[:,w].reshape(N)
