...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    h = np.ones((N, T, b.shape[0]))

    previous_h = h0
    for t in range(T):
        previous_h,_= rnn_step_forward(x[:,t,:], previous_h, Wx, Wh, b)
        h[:,t,:] = previous_h
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        ...
        if t == 0:
            pre = h0
        dx[:,t,:], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:,t,:] + dprev_h, (xt, pre, Wx, Wh, b, nxt))
        nxt = pre
