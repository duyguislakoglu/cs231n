...
class KNearestNeighbor(object):
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        # shape issue
        #repeated_x = np.repeat(X, self.X_train.shape[0], axis=0)
        #repeated_train = np.tile(self.X_train, (self.X_train.shape[0] ,1))
        #difference = np.abs(repeated_x - repeated_train)
        #difference = np.sum(difference, axis=1)
        #dists = np.reshape(difference, (-1 , self.X_train.shape[0]))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            closest_y = self.y_train[k_sorted_distances]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #########################################################################
            # TODO:                                                                 #
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            a, b = np.unique(closest_y, return_counts=True)
            y_pred[i] = a[b.argmax()]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return y_pred
