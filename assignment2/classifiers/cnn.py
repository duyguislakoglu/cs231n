...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        conv_forward_out_1, cache_forward_1 = conv_lrelu_pool_forward(X, self.params['W1'], self.params['b1'], conv_param,lrelu_param,pool_param)
        affine_forward_out_2, cache_forward_2 = affine_forward(conv_forward_out_1, self.params['W2'], self.params['b2'])
        affine_relu_2, cache_relu_2 = leaky_relu_forward(affine_forward_out_2,lrelu_param)
        ...
        def reg(x):
            return self.reg * x
        dl3, W3, b3= affine_backward(dout, cache_forward_3)
        dl2 = leaky_relu_backward(dl3, cache_relu_2)
        dl2, W2, b2 = affine_backward(dl2, cache_forward_2)
        dX1, W1, b1 = conv_lrelu_pool_backward(dl2, cache_forward_1)
        for_reg = np.sum(self.params["W1"] ** 2) + np.sum(self.params["W2"] ** 2) + np.sum(self.params["W3"] ** 2)

        grads.update({"W1": W1+ reg(self.params["W1"]),"W2": W2+ reg(self.params["W2"]),"W3": W3+ reg(self.params["W3"]), "b1": b1 , "b2":b2, "b3":b3})

        loss += reg(1/2 * (np.sum(for_reg)))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
