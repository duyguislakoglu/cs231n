...
def affine_backward(dout, cache):
    ...
    dw = np.transpose(x_reshaped).dot(dout)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_prime = int(1 + (H + 2 * padding - w.shape[-2]) / stride)
    W_prime = int(1 + (W + 2 * padding - w.shape[-1]) / stride)

    out = np.zeros((N , F , H_prime, W_prime))
    padded = np.pad(x, ((0,), (0,), (1,), (1,)), 'constant', constant_values=((0,0),(0,0),(0,0),(0,0)))

    for a in range(H_prime):
        for bi in range(W_prime):
            frame = padded[:, :, (a * stride):(a * stride + w.shape[-2]), (bi * stride):(bi * stride + w.shape[-1])]
            for c in range(F):
                out[:, c , a, bi] = np.sum(frame * w[c, :, :, :], axis=(1,2,3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for k in range(H_prime):
                for l in range(W_prime):
                    a = int(np.argmax(np.mat( x[i, j, k * stride : k * stride + HH, l * stride : l * stride + WW]))/HH)+k * stride
                    b = int(np.argmax(np.mat(x[i, j, k * stride : k * stride + HH, l * stride : l * stride + WW])) % WW)+l * stride
                    dx[i, j, a ,b] += dout[i,j,k,l]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
